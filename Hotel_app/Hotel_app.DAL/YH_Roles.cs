...
namespace Hotel_app.DAL
{
	...
	public partial class YH_Roles
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "YH_Roles"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from YH_Roles");
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into YH_Roles(");
			strSql.Append("yydh,qymc,R_lsbh,R_RolesName,R_Bz,R_ts,is_top,is_select)");
			...
			parameters[4].Value = model.R_Bz;
			parameters[5].Value = model.R_ts;
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update YH_Roles set ");
			strSql.Append("yydh=@yydh,");
			strSql.Append("qymc=@qymc,");
			strSql.Append("R_lsbh=@R_lsbh,");
			strSql.Append("R_RolesName=@R_RolesName,");
			strSql.Append("R_Bz=@R_Bz,");
			strSql.Append("R_ts=@R_ts,");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from YH_Roles ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		public Hotel_app.Model.YH_Roles GetModel(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,yydh,qymc,R_lsbh,R_RolesName,R_Bz,R_ts,is_top,is_select from YH_Roles ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			Hotel_app.Model.YH_Roles model=new Hotel_app.Model.YH_Roles();
			...
			if(ds.Tables[0].Rows.Count>0)
			{
				...
				if(ds.Tables[0].Rows[0]["R_Bz"]!=null && ds.Tables[0].Rows[0]["R_Bz"].ToString()!="")
				{
					model.R_Bz=ds.Tables[0].Rows[0]["R_Bz"].ToString();
				}
				if(ds.Tables[0].Rows[0]["R_ts"]!=null && ds.Tables[0].Rows[0]["R_ts"].ToString()!="")
				{
					model.R_ts=int.Parse(ds.Tables[0].Rows[0]["R_ts"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,yydh,qymc,R_lsbh,R_RolesName,R_Bz,R_ts,is_top,is_select ");
			strSql.Append(" FROM YH_Roles ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,yydh,qymc,R_lsbh,R_RolesName,R_Bz,R_ts,is_top,is_select ");
			strSql.Append(" FROM YH_Roles ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from YH_Roles T ");
