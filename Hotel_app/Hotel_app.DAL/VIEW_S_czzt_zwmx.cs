...
namespace Hotel_app.DAL
{
	...
	public partial class VIEW_S_czzt_zwmx
	{
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from VIEW_S_czzt_zwmx ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select jzbh,czzt ");
			strSql.Append(" FROM VIEW_S_czzt_zwmx ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM VIEW_S_czzt_zwmx ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from VIEW_S_czzt_zwmx T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "VIEW_S_czzt_zwmx";
