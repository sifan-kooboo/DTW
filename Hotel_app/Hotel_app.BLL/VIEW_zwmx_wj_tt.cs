...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.VIEW_zwmx_wj_tt dal=new Hotel_app.DAL.VIEW_zwmx_wj_tt();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.VIEW_zwmx_wj_tt GetModelByCache()
		{
			//该表无主键信息，请自定义主键/条件字段
			string CacheKey = "VIEW_zwmx_wj_ttModel-" ;
			...
			return (Hotel_app.Model.VIEW_zwmx_wj_tt)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VIEW_zwmx_wj_tt> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VIEW_zwmx_wj_tt> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.VIEW_zwmx_wj_tt> modelList = new List<Hotel_app.Model.VIEW_zwmx_wj_tt>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.VIEW_zwmx_wj_tt model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.VIEW_zwmx_wj_tt();
					...
					if(dt.Rows[n]["krxm_tt"]!=null && dt.Rows[n]["krxm_tt"].ToString()!="")
					{
					model.krxm_tt=dt.Rows[n]["krxm_tt"].ToString();
					}
					modelList.Add(model);
				}
			}
