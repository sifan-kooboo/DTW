...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.VS_syxfmx_xsy dal=new Hotel_app.DAL.VS_syxfmx_xsy();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.VS_syxfmx_xsy GetModelByCache()
		{
			//该表无主键信息，请自定义主键/条件字段
			string CacheKey = "VS_syxfmx_xsyModel-" ;
			...
			return (Hotel_app.Model.VS_syxfmx_xsy)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VS_syxfmx_xsy> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VS_syxfmx_xsy> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.VS_syxfmx_xsy> modelList = new List<Hotel_app.Model.VS_syxfmx_xsy>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.VS_syxfmx_xsy model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.VS_syxfmx_xsy();
					...
					{
						model.Expr2=decimal.Parse(dt.Rows[n]["Expr2"].ToString());
					}
					if(dt.Rows[n]["Expr3"]!=null && dt.Rows[n]["Expr3"].ToString()!="")
					{
						model.Expr3=decimal.Parse(dt.Rows[n]["Expr3"].ToString());
					}
