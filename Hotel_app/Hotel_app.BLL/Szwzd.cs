...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.Szwzd dal=new Hotel_app.DAL.Szwzd();
		public Szwzd()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.Szwzd GetModelByCache(int id)
		{
			
			string CacheKey = "SzwzdModel-" + id;
			...
			return (Hotel_app.Model.Szwzd)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Szwzd> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Szwzd> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.Szwzd> modelList = new List<Hotel_app.Model.Szwzd>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.Szwzd model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.Szwzd();
					...
					if(dt.Rows[n]["is_ffzf"]!=null && dt.Rows[n]["is_ffzf"].ToString()!="")
					{
						if((dt.Rows[n]["is_ffzf"].ToString()=="1")||(dt.Rows[n]["is_ffzf"].ToString().ToLower()=="true"))
						{
						model.is_ffzf=true;
						}
						else
