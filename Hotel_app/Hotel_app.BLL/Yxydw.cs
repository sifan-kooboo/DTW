...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.Yxydw dal=new Hotel_app.DAL.Yxydw();
		public Yxydw()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.Yxydw GetModelByCache(int id)
		{
			...
			return (Hotel_app.Model.Yxydw)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Yxydw> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Yxydw> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.Yxydw> modelList = new List<Hotel_app.Model.Yxydw>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.Yxydw model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.Yxydw();
					...
					if(dt.Rows[n]["xsed"]!=null && dt.Rows[n]["xsed"].ToString()!="")
					{
						model.xsed=decimal.Parse(dt.Rows[n]["xsed"].ToString());
					}
					modelList.Add(model);
				}
			}
