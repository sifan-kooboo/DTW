...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.BB_g_j_mxfx_ls dal=new Hotel_app.DAL.BB_g_j_mxfx_ls();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.BB_g_j_mxfx_ls GetModelByCache(int id)
		{
			
			string CacheKey = "BB_g_j_mxfx_lsModel-" + id;
			...
			return (Hotel_app.Model.BB_g_j_mxfx_ls)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.BB_g_j_mxfx_ls> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.BB_g_j_mxfx_ls> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.BB_g_j_mxfx_ls> modelList = new List<Hotel_app.Model.BB_g_j_mxfx_ls>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.BB_g_j_mxfx_ls model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.BB_g_j_mxfx_ls();
