...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.View_Qskzd_fjzt dal=new Hotel_app.DAL.View_Qskzd_fjzt();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.View_Qskzd_fjzt GetModelByCache()
		{
			//该表无主键信息，请自定义主键/条件字段
			string CacheKey = "View_Qskzd_fjztModel-" ;
			...
			return (Hotel_app.Model.View_Qskzd_fjzt)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.View_Qskzd_fjzt> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.View_Qskzd_fjzt> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.View_Qskzd_fjzt> modelList = new List<Hotel_app.Model.View_Qskzd_fjzt>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.View_Qskzd_fjzt model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.View_Qskzd_fjzt();
					...
					if(dt.Rows[n]["zyzt_zd"]!=null && dt.Rows[n]["zyzt_zd"].ToString()!="")
					{
					model.zyzt_zd=dt.Rows[n]["zyzt_zd"].ToString();
					}
