...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.Sjzmx dal=new Hotel_app.DAL.Sjzmx();
		public Sjzmx()
		{}
		#region  Method
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.Sjzmx GetModelByCache(int id)
		{
			
			string CacheKey = "SjzmxModel-" + id;
			...
			return (Hotel_app.Model.Sjzmx)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Sjzmx> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.Sjzmx> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.Sjzmx> modelList = new List<Hotel_app.Model.Sjzmx>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.Sjzmx model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.Sjzmx();
