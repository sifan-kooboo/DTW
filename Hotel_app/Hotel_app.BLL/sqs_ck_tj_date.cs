...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.sqs_ck_tj_date dal=new Hotel_app.DAL.sqs_ck_tj_date();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.sqs_ck_tj_date GetModelByCache(int id)
		{
			
			string CacheKey = "sqs_ck_tj_dateModel-" + id;
			...
			return (Hotel_app.Model.sqs_ck_tj_date)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.sqs_ck_tj_date> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.sqs_ck_tj_date> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.sqs_ck_tj_date> modelList = new List<Hotel_app.Model.sqs_ck_tj_date>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.sqs_ck_tj_date model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.sqs_ck_tj_date();
					...
					if(dt.Rows[n]["tj_rq"]!=null && dt.Rows[n]["tj_rq"].ToString()!="")
					{
						model.tj_rq=DateTime.Parse(dt.Rows[n]["tj_rq"].ToString());
					}
