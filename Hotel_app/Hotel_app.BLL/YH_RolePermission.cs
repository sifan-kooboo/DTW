...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.YH_RolePermission dal=new Hotel_app.DAL.YH_RolePermission();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.YH_RolePermission GetModelByCache(int ID)
		{
			
			string CacheKey = "YH_RolePermissionModel-" + ID;
			...
			return (Hotel_app.Model.YH_RolePermission)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.YH_RolePermission> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.YH_RolePermission> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.YH_RolePermission> modelList = new List<Hotel_app.Model.YH_RolePermission>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.YH_RolePermission model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.YH_RolePermission();
					...
					if(dt.Rows[n]["p_lsbh"]!=null && dt.Rows[n]["p_lsbh"].ToString()!="")
					{
					model.p_lsbh=dt.Rows[n]["p_lsbh"].ToString();
					}
					if(dt.Rows[n]["A_Appdr"]!=null && dt.Rows[n]["A_Appdr"].ToString()!="")
					{
					model.A_Appdr=dt.Rows[n]["A_Appdr"].ToString();
					}
					if(dt.Rows[n]["A_AppName"]!=null && dt.Rows[n]["A_AppName"].ToString()!="")
					{
					model.A_AppName=dt.Rows[n]["A_AppName"].ToString();
					}
					if(dt.Rows[n]["P_Value"]!=null && dt.Rows[n]["P_Value"].ToString()!="")
					{
						if((dt.Rows[n]["P_Value"].ToString()=="1")||(dt.Rows[n]["P_Value"].ToString().ToLower()=="true"))
						{
						model.P_Value=true;
						}
						else
