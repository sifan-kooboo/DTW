...
namespace Hotel_app.BLL
{
	...
	{
		private readonly Hotel_app.DAL.VIEW_Qttyd_bak dal=new Hotel_app.DAL.VIEW_Qttyd_bak();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public Hotel_app.Model.VIEW_Qttyd_bak GetModelByCache()
		{
			//该表无主键信息，请自定义主键/条件字段
			string CacheKey = "VIEW_Qttyd_bakModel-" ;
			...
			return (Hotel_app.Model.VIEW_Qttyd_bak)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VIEW_Qttyd_bak> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Hotel_app.Model.VIEW_Qttyd_bak> DataTableToList(DataTable dt)
		{
			List<Hotel_app.Model.VIEW_Qttyd_bak> modelList = new List<Hotel_app.Model.VIEW_Qttyd_bak>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Hotel_app.Model.VIEW_Qttyd_bak model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Hotel_app.Model.VIEW_Qttyd_bak();
