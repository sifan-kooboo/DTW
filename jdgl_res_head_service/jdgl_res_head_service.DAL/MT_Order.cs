...
namespace jdgl_res_head_service.DAL
{
	...
	public partial class MT_Order
	{
		...
		public jdgl_res_head_service.Model.MT_Order GetModel(int id)
		{
			...
			jdgl_res_head_service.Model.MT_Order model=new jdgl_res_head_service.Model.MT_Order();
		...
		/// 得到一个对象实体
		/// </summary>
		public jdgl_res_head_service.Model.MT_Order DataRowToModel(DataRow row)
		{
			jdgl_res_head_service.Model.MT_Order model=new jdgl_res_head_service.Model.MT_Order();
			if (row != null)
			{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update MT_Order set " + strValue);
            strSql.Append(" where order_lsbh='" + order_lsbh+"' ");
