...
namespace jdgl_res_head_service.Qyddj
{
    ...
    public class Qyddj_app : System.Web.Services.WebService
    {
        ...
        [WebMethod(Description = "供门店下载网上的预订信息")]
        public string yddj_download_DS_test(string qymc, out  int rows,out  bool status, out DataSet DS_download)
        {
            string s = common_file.common_app.get_failure;
            rows = 0;
            status = false;
            ...
            DataSet ds_skyd;
            web_skyd web_skyd_new = new web_skyd();
            ds_skyd = web_skyd_new.yddj_download_DS_Test(qymc,out rows, out status);
            if (ds_skyd != null && rows > 0)
            {
                DS_download = ds_skyd;
                s = common_file.common_app.get_suc;
