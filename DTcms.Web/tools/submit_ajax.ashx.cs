...
namespace DTcms.Web.tools
{
    ...
    public class submit_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                ...
                case "view_cart_count": //输出当前购物车总数
                    ...
                    break;
                case "weixin_getconfig"://获取微信js-sdk需要的参数
                    weixin_getconfig(context);
                    break;
            }
        }
        ...
        #region 获取微信JS-SDK需要的参数
        private void weixin_getconfig(HttpContext context)
        {
            string result = "";
            int accountId = 0;
            int.TryParse(context.Request["accountId"], out accountId);
            ...
            if (accountModel != null)
            {
                ...
                string ticket = AccessTokenContainer.TryGetJsApiTicket(accountModel.appid, accountModel.appsecret);
                //获取签名
                var signature = Senparc.Weixin.MP.Helpers.JSSDKHelper.GetSignature(ticket, nonceStr, timestamp, context.Request.Url.AbsoluteUri);
                result = string.Format("{\"appId\":\"{0}\",\"Timestamp\":\"{1}\",\"NonceStr\":\"{2}\",\"Signature\":\"{3}\"}"
                , accountModel.appid, timestamp, nonceStr, signature);
            }
            context.Response.Write(result);
        }
        #endregion
